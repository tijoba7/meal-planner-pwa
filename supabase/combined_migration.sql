...
-- ─── Server-side rate limiting (H-2 fix) ────────────────────────────────────
...
create policy "scrape_usage: user can insert own"
  on public.scrape_usage for insert to authenticated
  with check (user_id = auth.uid());

create policy "scrape_usage: user can read own"
  on public.scrape_usage for select to authenticated
  using (user_id = auth.uid());
...
declare
  ...
  v_max int; v_count int; v_oldest timestamptz;
begin
  if v_user_id is null then
    return jsonb_build_object('allowed', false, 'remaining', 0, 'retry_after_sec', 0);
  end if;
  select coalesce((value)::int, 10) into v_max
    from public.app_settings where key = 'scraping.rate_limit';
  ...
  if v_count >= v_max then
    return jsonb_build_object('allowed', false, 'remaining', 0,
      'retry_after_sec', ceil(extract(epoch from (v_oldest + interval '1 hour' - now()))));
  end if;
  insert into public.scrape_usage (user_id) values (v_user_id);
  return jsonb_build_object('allowed', true, 'remaining', v_max - v_count - 1, 'retry_after_sec', 0);
end;
$$;
